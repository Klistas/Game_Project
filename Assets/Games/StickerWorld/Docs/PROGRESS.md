...
## 2026-07-09 — 3D G0 카메라/가독성 품질 1차 패스

- [x] Stage 01/02/03 카메라를 기존 원거리 조감보다 조금 가까운 고정 쿼터뷰로 조정.
  - `orthographicSize`: 5.9 -> 5.55
  - 카메라 높이/후방 위치/각도를 함께 조정해 화면 잘림을 줄임.
- [x] 카메라별 URP 추가 데이터를 자동 생성하도록 씬 빌더 수정.
  - Post Processing 활성화
  - SMAA High 안티앨리어싱 적용
  - Depth Texture 요청
  - HDR/MSAA 허용
- [x] 월드 라벨 스케일을 키우고 메인 카메라 방향을 따르도록 변경.
  - 라벨 스케일: 0.045 -> 0.055
  - 얇은 오브젝트 스케일 상속 방지 유지
- [x] Directional Light에 소프트 그림자와 약한 그림자 강도 추가.
- [x] Stage 01/02/03 씬 빌더 재실행.
- [x] 화면 확인 스크린샷 저장.
  - `Assets/Screenshots/stickerworld_camera_quality_stage03_final.png`
- [x] EditMode 테스트 45/45 통과.
- [x] 최종 콘솔 에러 0건, 관련 경고 0건 확인.
...
- [x] 3D 카메라 구도/안티앨리어싱/라벨 가독성 1차 개선.
